...
# Defaulting to SQLite for easy setup, but SQLAlchemy allows easy swap to PostgreSQL
...
class UserActivity(Base):
    ...
    action = Column(String) # e.g., "login", "download", "upload", "file_access", "failed_login"
...
# In-memory models cache for simple UBA per user (In prod, persist these models)
uba_models = {}
...
def train_user_baseline(db, user_id):
    """Train the Isolation Forest model for a specific user using their historical data."""
    activities = db.query(UserActivity).filter(UserActivity.user_id == user_id).all()
    if len(activities) < 10:
        return # Not enough data to train a reliable baseline
...
def detect_anomaly(db, user_id, activity):
    ...
    if model is None:
        # Fallback simple rules if no model
        score = 0.0
        is_anom = False
        if activity.action == "failed_login": score += 30
        if activity.data_transferred_mb > 1000: score += 50
        if activity.timestamp.hour < 5 or activity.timestamp.hour > 23: score += 20
        is_anom = score >= 50
        return is_anom, score

    # Model prediction: -1 for anomaly, 1 for normal
    pred = model.predict([features])[0]
    
    # decision_function gives lower values for anomalies
    raw_score = model.decision_function([features])[0]
    
    # Normalize score somewhat to 0-100 where higher is more anomalous
    normalized_score = max(0, min(100, (-raw_score + 0.5) * 100))
    
    # Add rule-based penalties
    if activity.action == "failed_login": normalized_score = min(100, normalized_score + 40)
    if activity.data_transferred_mb > 5000: normalized_score = min(100, normalized_score + 50)
    
    is_anom = bool(pred == -1 or normalized_score > 60)
    return is_anom, normalized_score

def update_user_risk(db, user_id, anomaly_score):
    """Update the aggregate risk score for a user."""
    risk = db.query(UserRisk).filter(UserRisk.user_id == user_id).first()
    if not risk:
        risk = UserRisk(user_id=user_id, current_risk_score=0.0)
        db.add(risk)
    
    # Smooth update: 70% old risk, 30% new anomaly score
...
def get_dashboard_data():
    ...
    try:
        ...
        # High Risk Users
        high_risk = db.query(UserRisk).filter(UserRisk.risk_level.in_(["High", "Medium"])).order_by(UserRisk.current_risk_score.desc()).all()
        
        # Recent Anomalies for alerts
        anomalies = db.query(UserActivity).filter(UserActivity.is_anomalous == True).order_by(UserActivity.timestamp.desc()).limit(10).all()
        
        return jsonify({
            "success": True,
            "recent_activities": [{
                "id": a.id, "user_id": a.user_id, "action": a.action, "time": a.timestamp.isoformat(),
                "is_anomalous": a.is_anomalous, "score": a.anomaly_score, "risk_level": a.risk_level
            } for a in recent],
            "risk_scores": [{
                "user_id": r.user_id, "score": round(r.current_risk_score, 2), "level": r.risk_level
            } for r in high_risk],
            "alerts": [{
                "user_id": a.user_id, "action": a.action, "time": a.timestamp.isoformat(),
...
def simulate_data():
    """Endpoint to inject sample scenarios for the demo."""
    db = next(get_db())
    data = request.json
    scenario = data.get('scenario', 'normal')
    user_id = data.get('user_id', 'jdoe')
    
    # Generate some normal baseline history first if empty
    ...
    if history < 15:
        ...
        for i in range(15):
            db.add(UserActivity(
                user_id=user_id,
                action="login",
                timestamp=base_time + timedelta(days=i*0.3, hours=random.randint(9, 17)), # Normal hours
    ...
    if scenario == "3am_login":
        ...
        for i in range(5):
            activities_to_add.append(UserActivity(
                user_id=user_id, action="failed_login", timestamp=now - timedelta(minutes=5-i),
    ...
    for act in activities_to_add:
        ...
        if score > 70: act.risk_level = "High"
        elif score > 40: act.risk_level = "Medium"
        else: act.risk_level = "Low"
...
def setup_uba(app):
    ...
    print("UBA Module Initialized and Registered.")
